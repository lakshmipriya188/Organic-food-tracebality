    ...
    with col_actions:
        ...
        if not is_logged_in:
            ...
            if is_admin:
                ...
                # Customer navigation: ONLY Ask Me, Wishlist, Cart, and Profile
                act_col1, act_col2, act_col3, act_col4 = st.columns([1.3, 1.2, 1.1, 1.2])
                with act_col1:
                    if st.button("✨ Ask Me", key="hdr_askme_btn", use_container_width=True):
                        go_to("ml_prediction")
                        st.rerun()
                with act_col2:
                    if st.button(w_label, key="hdr_wish_btn", use_container_width=True):
